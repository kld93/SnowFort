...
poc %>% 
  ...
  annotate("text", x = 7.15, y = 0.05, label = "FRESHLY FALLEN SNOW", vjust = -1, color = "steelblue1", fontface = "bold") +
  geom_hline(yintercept = 0.3, linetype = "dashed", color = "steelblue1", linewidth = 1) +
  annotate("text", x = 7.15, y = 0.3, label = "SETTLED SNOW", vjust = -1, color = "steelblue1", fontface = "bold") +
  geom_hline(yintercept = 0.4, linetype = "dashed", color = "steelblue3", linewidth = 1) +
  annotate("text", x = 7.15, y = 0.4, label = "FIRN", vjust = -1, color = "steelblue3", fontface = "bold") +
  geom_hline(yintercept = 0.83, linetype = "dashed", color = "steelblue4", linewidth = 1) +
  annotate("text", x = 7.15, y = 0.83, label = "ICE (w/ bubbles)", vjust = -1, color = "steelblue4", fontface = "bold") +
  geom_hline(yintercept = 0.917, linetype = "dashed", color = "steelblue4", linewidth = 1) +
  annotate("text", x = 7.15, y = 0.917, label = "PURE GLACIER ICE (no bubbles)", vjust = -1, color = "steelblue4", fontface = "bold") +
  scale_y_continuous(trans = "reverse", name = "CSB Density (g/cm3)", limits = c(1.025, 0)) +
  xlab("CSB Height (cm)") +
  labs(color = "CSB Mass (g)") +
  theme_bw() +
  theme(legend.position = c(0.90, 0.42),
